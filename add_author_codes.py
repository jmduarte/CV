...
for l in t:
    ...
    is_recent = (int(l["year"]) >= 2020) or (
        int(l["year"]) == 2019 and "month" in l and int(l["month"]) >= 7
    )
    if not is_recent:
        continue
    ...
    if l["ID"] in author_codes:
        ...
        "collaboration" in l
        and "CMS" in l["collaboration"]
        or ("usera" in l and "CMS" in l["usera"])
    ):
