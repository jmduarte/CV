...
\begin{figure}[htb!]
    ...
    \caption[Jet tagging]{
        A visual representation of a collision event at the LHC and the task of jet tagging~[\mycite{Duarte:2024xkc}].
...
For example, the connection between measuring the rate of $\PH\PH$ production via \emph{vector boson fusion} and the $\kappa_{2\PV}$ coupling modifier is illustrated in Fig.~\ref{fig:higgs}.
...
\begin{figure}[htpb]
    ...
    \caption[Higgs boson pair production]{
        ...
        Candidate event in CMS of highly energetic $\PH\PH$ production via vector boson fusion (lower left)~[\mycite{CMS-PAS-HIG-23-012}].
...
\begin{figure}[htbp]
    ...
    \caption[HH4b and HHbb4q final states]{
        Merged $\PH\PH\to4\PQb$ (upper left) and merged $\PH\PH\to\bbbar\PW\PW\to\bbbar 4\Pq$ (upper right) final states~[\mycite{CMS:2025ngq}].
...
My previous work within the CMS Collaboration published in \emph{Phys. Rev. Lett.} established for the first time the high-\pt, merged $\PH\PH\to4\PQb$ final state as one of the most sensitive channels using Run~2~[\mycite{CMS:2022nmn}].
To improve the sensitivity of this search with Run~3 data, my former graduate student Raghav Kansal and I along with CMS collaborators developed a ``global particle transformer'' algorithm to classify a highly granular multitude of jet types, as shown in Fig.~\ref{fig:glopart}~[\mycite{Li:2024htp}, \mycite{CMS:2026uph}].
This builds on my prior work on one of the first graph neural networks (GNNs) for jet tagging~[\mycite{Moreno:2019bmu}, \mycite{Moreno:2019neq}].
...
With this improvement, my graduate student Daniel Primosch and I along with collaborators led a search for $\PH\PH\to4\PQb$ production through gluon fusion and vector boson fusion with Run-3 data recently submitted to \emph{Phys. Rev. D}~[\mycite{CMS:2026znu}].
...
\begin{figure}[htbp]
    ...
    \caption[Global particle transformer]{
        Diagram of the global particle transformer model architecture (upper)~[\mycite{CMS:2026uph}].
...
This search also used one of the first the transformer-based algorithm to identify the Higgs boson decay, and was one of the most sensitive in CMS to the coupling between two vector bosons and two Higgs bosons $\kappa_{2\PV}$~[\mycite{CMS-PAS-HIG-23-012}].
Together with other CMS collaborators, I contributed to the combination of all Run~2 Higgs boson pair producerion searches using Run~2 data, which included this channel, and was submitted to \emph{J. Phys. G.}~[\mycite{CMS:2025ngq}].
Finally, the same techniques were applied to search for heavy scalar resonance \PX decaying to a Higgs boson (\PH) and a Higgs-like scalar boson (\PY) in the two bottom quark ($\PH\to\bbbar$) and four quark ($\PY \to \PV\PV \to 4\Pq$) final state in a manuscript recently accepted by \emph{J. High Energy Phys.}~[\mycite{CMS:2026shw}].
...
This work was published in \emph{J. High Energy Phys.}~[\mycite{Li:2024qfq}].
A version of this algorithm has been adopted in the first CMS search for triple Higgs boson production, which we co-authored and will be submitted to \emph{Phys. Rev. Lett.} shortly~[\mycite{CMS-PAS-HIG-24-012}].
The results of this search set the most stringent constraints to date on $\PH\PH\PH$ production and on the quartic self-interaction, while establishing an innovative strategy for Higgs boson self-coupling measurements at the LHC and HL-LHC.
...
\begin{figure}
    ...
    \caption[SPANet HHH reconstruction and search]{
        The performance of the enhanced SPANet algorithm that considers both resolved and merged jet reconstruction possibilities for $\PH\PH\PH$ production compared to the baseline (left)~[\mycite{Li:2024qfq}].
        ...
        Post-fit yields in the CMS search for $\PH\PH\PH$ production signal-region categories with three reconstructed Higgs bosons (right)~[\mycite{CMS-PAS-HIG-24-012}].
...
This search was combined with a search in the all-hadronic channel and one focusing on $\PV\PH$ production in the one-lepton channel, and was recently submitted to \emph{J. High Energy Phys.}~[\mycite{CMS:2026yrb}].
...
Building on our previous work searching for LLPs with this unique muon detector shower (MDS) signature~[\mycite{CMS:2021juv}, \mycite{CMS:2024bvl}, \mycite{CMS:2024ake}], the new search targets similar hadronic decays of the LLP in the \PB parking data set.
The search, recently published in \emph{Phys. Rev. D}~[\mycite{CMS:2025rtd}], yields the most stringent limits to date on the decay of a long-lived scalar particle to a pair of hadrons when the LLP is produced by a bottom hadron, particularly for long decay lengths.
% We have also contributed to several other searches for LLPs~[\mycite{CMS:2022wjc}] and boosted dijet resonances~[\mycite{CMS-PAS-EXO-24-007}].
...
A novel ML algorithm I helped develop in Ref.~[\mycite{Pata:2021oez}], published in \emph{Eur. Phys. J. C}, uses a ML to improve the particle-flow (PF) algorithm, known as the MLPF.
...
In previous work, we have shown this approach can be extended to future proposed colliders and detectors, such as the Compact Linear Collider (CLIC), in work published in \emph{Comm. Phys.}~[\mycite{Pata:2023rhh}].
...
This work was published in \emph{Phys. Rev. D} with former graduate student Farouk Mokhtar as first author~[\mycite{Mokhtar:2025zqs}].
In addition, we evaluated the performance of the MLPF algorithm in real CMS data and simulation for the first time in a paper accepted by \emph{Eur. Phy. J. C}~[\mycite{CMS:2026znb}].
...
Finally, we found that the per-particle latent representations of MLPF can be used to improve the performance of downstream tasks, such as jet tagging~[\mycite{Mokhtar:2026gdb}].
...
\begin{figure}[h!]
    ...
    \caption[MLPF performance]{
        The jet energy resolution as a function of the fine-tuning dataset size, for the fine-tuned MLPF model (orange), a model trained from scratch (blue), and the traditional PF algorithm (grey) in electron-positron collisions~[\mycite{Mokhtar:2025zqs}].
        The fine-tuned model is able to outperform PF with a much smaller dataset size compared to the from-scratch model.
        The runtime of the baseline PF reconstruction compared to the MLPF inference, demonstrating the improved computational performance of MLPF in CMS software (right)~[\mycite{CMS:2026znb}].
    }
    \label{fig:mlpf}
\end{figure}
...
We have studied and developed different pre-training strategies based on scaling up a simple framework for contrastive learning of representations (SimCLR)~[\mycite{Zhao:2024kry}], training a fine-griained classifier~[\mycite{Li:2024htp}], and a joint-embedding predictive architecture (JEPA), which predicts representations of ``masked'' particles inside a jet using the surrounding particles as the context~[\mycite{Katel:2024ygn}].
With collaborators, I also contributed to developing a contrastive learning approach based on different jet clustering algorithms and the renormalization group~[\mycite{Hao:2025abk}].
We have also studied efficient and interpretable algorithms based on transformers for charged particle tracking~[\mycite{Miao:2024oqy}, \mycite{Govil:2025nvy}, \mycite{Miao:2026yvl}] and jet tagging~[\mycite{Bhatnagar:2024fkx}, \mycite{Legge:2025cnm}, \mycite{Wang:2025dky}, \mycite{Wang:2026lqb}].
Several of these works have been accepted at ICML and the Machine Learning and the Physical Sciences (ML4PS) Workshop at the Neural Information Processing Systems (NeurIPS) conference.
...
\begin{figure}[h!]
    ...
    \caption[Foundation model for HEP]{Schematic of a foundation model based on HEP data~[\mycite{Duarte:2025qbk}].
...
Postdoctoral fellow Melissa Quinnan, graduate student Ellison Scheuller, and I have helped implement this anomaly detection algorithm into data-taking during LHC Run 3~[\mycite{CMS-DP-2023-079}, \mycite{CMS-DP-2024-059}, \mycite{CMS-DP-2025-061}] based on prior work published in \emph{Nat. Mach. Intell.}~[\mycite{Govorkova:2021utb}].
Previously, with former undergraduate student Zichun Hao and former graduate student Raghav Kansal, we developed the first Lorentz-equivariant autoencoder for particle physics published in \emph{Eur. Phys. J. C}~[\mycite{Hao:2022zns}].
...
Building on our previous work in the 2021 NeurIPS conference~[\mycite{Kansal:2021cqp}], we have developed a new more efficient transformer-based generative algorithm and comprehensive metrics to compare them, in work published in \emph{Phys. Rev. D}~[\mycite{Kansal:2022spb}].
A workshop paper describing the improved efficient algorithm was also accepted to the NeurIPS Workshop on Machine Learning and the Physical Sciences in 2023~[\mycite{Li:2024xpw}].
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.7\textwidth]{LHC_Event_Processing.pdf}
...
I contributed to the design, implementation, and evaluation of real-time AI algorithms on FPGAs and ASICs for the LHC trigger and data acquisition~[\mycite{Gonski:2026jgu}].
...
My key contribution is the creation~[\mycite{Duarte:2018ite}] and continued development~[\mycite{Schulte:2025mai}] of \texttt{hls4ml}, a flexible, open-source platform for converting high-level neural networks from PyTorch, Keras, or similar frameworks into high-level synthesis (HLS) kernels suitable for deployment on FPGAs and ASICs.
...
Recently, we developed new FPGA implementations of deep sets and graph neural networks for jet tagging in the LHC trigger, and compared their performance~[\mycite{Odagiu:2024bkp}].
...
This approach was demonstrated on various tasks, including Bragg peak finding and jet classification, achieving models with improved accuracy, smaller latencies, or reduced resource utilization, quantified through a proxy metric known as bit operations (BOPs), relative to the baseline models~[\mycite{Weitz:2025tcj}].
...
Specifically, the Surrogate Neural Architecture Codesign Package (SNAC-Pack)~[\mycite{Weitz:2025dia}, \mycite{Weitz:2026xsv}] runs a multi-objective parallelized search using a hardware surrogate model, known as \texttt{wa-hls4ml}~[\mycite{Hawks:2025hew}], which outputs per-trial resource and latency estimates, avoiding the synthesis cost that would otherwise dominate the search loop.
% An agentic frontend lets users run the pipeline on new datasets without modifying the framework.
SNAC-Pack was demonstrated on a qubit readout task, reducing the design space exploration process from months of manual fine-tuning to hours of automated search, and was accepted at the 2025 NeurIPS ML4PS Workshop and 2026 International Conference on Automated Machine Learning (AutoML) Applications, Benchmarks, Challenges, and Datasets (ABCD) Track.

With collaborators in the CSE Department and led by graduate student Olivia Weng, we have developed efficient hardware implementations of neural networks with skip connections~[\mycite{Weng:2023tailor}], improved the robustness of neural networks to faults induced by radiation~[\mycite{Weng:2024fkeras}], and implemented ensembles of neural networks as lookup tables to improve their scalability~[\mycite{Weng:2025amigolut}].
...
I contributed to the integration of these workflows into the CMS computing framework, which is documented in a paper pulished in \emph{Comput. Softw. Big Sci.}~[\mycite{CMS:2024twn}].
We also recently developed a framework for deploying the necessary server infrastructure as a GPU-equipped Kubernetes cluster~[\mycite{Kondratyev:2025jfj}].
...
Timothy Legge also received the 2026 Shang-Keng Ma Memorial Award from the Department of Physics, which is given to a graduating physics student who has shown exceptional ability and promise.
...
I have served as a peer reviewer for \emph{J. High Energy Phys.}, \emph{Phys. Lett. B}, \emph{Phys. Rev. X}, \emph{Phys. Rev. D}, \emph{Phys. Rev. Research}, \emph{Eur. Phys. J. C}, \emph{Comput. Softw. Big Sci.}, and \emph{Nucl. Instrum. Methods Phys. Res. A}, and \emph{Applied Optics}, and as a Guest Associate Editor for \emph{Front. Big Data} and \emph{Front. AI}.
...
I am one of the Energy Frontier Track Convener for APS Division of Particles and Fields (DPF) Meeting in 2026 and I am a member of the Program Committee for the US Muon Collider Collaboration Meeting in 2026.
