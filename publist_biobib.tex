...
\DeclareSourcemap{
    \maps[datatype=bibtex]{
      \map{
        \step[fieldsource=contribution, final=true]
        \step[fieldset=userc, origfieldval, final=true]
    }
  }
}

% for UCSD, add contribution codes and statement to recent pubs
...
\renewbibmacro*{note}{
    ...
    {
      \printtext[bold]{Contribution:} \printfield{userc} \newunit
    }
    \iffieldundef{userb}{
    }
    {
...
\begin{resume}
  ...
  % \printbibliography[heading=none,keyword={career}] % to print just career

  \subsection{II. Review and Invited Articles}
  \newrefsection[bib_reviews]
  \nocite{*}
  \vspace{10pt}
  \printbibliography[heading=none]
